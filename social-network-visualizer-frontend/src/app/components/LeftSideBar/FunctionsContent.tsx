      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div className="w-[90%] md:w-[600px] p-4 bg-black/60 backdrop-blur-md border border-white/20 rounded-md relative">
            <h3 className="text-lg font-semibold text-center mb-2">Find shortest path</h3>
            <p className="text-sm text-center text-white/80 mb-4">Provide source and target usernames</p>
            ...
            <p className="text-xs text-center text-white/60">Leaving fields empty shows the default graph.</p>

            <button onClick={() => setIsModalOpen(false)} className="absolute top-2 right-3 text-white text-lg font-bold">
              &times;
            </button>
          </div>
        </div>
