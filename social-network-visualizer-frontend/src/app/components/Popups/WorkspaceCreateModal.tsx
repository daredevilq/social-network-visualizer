...
export default function WorkspaceCreateModal(props: WorkspaceCreateModalProps) {
  ...
  const handleCreate = () => {
    ...
    const nameExists = workspaceList.some((workspace) => workspace.toLowerCase() === trimmedName.toLowerCase());
  ...
  return (
    <Dialog open={open} onClose={onCancel} className="fixed inset-0 z-50 flex items-center justify-center">
      {open && <div className="fixed inset-0 bg-black/50" aria-hidden="true" onClick={onCancel} />}

      <div className="bg-[#262631] rounded-xl p-6 w-full max-w-md z-50 relative shadow-xl text-white" onClick={(e) => e.stopPropagation()}>
        <Dialog.Title className="text-2xl font-bold text-center mb-4">Create Workspace</Dialog.Title>
        ...
        <div className="flex justify-end space-x-2">
          ...
          <button onClick={handleCreate} className="px-4 py-2 rounded bg-[#7140F4] hover:bg-[#5b30c9]">
            Create
          </button>
        </div>
      </div>
