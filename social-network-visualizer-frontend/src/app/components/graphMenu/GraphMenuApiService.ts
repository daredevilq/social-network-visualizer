...
export class GraphApiService {
  static async removeNodeFromWorkspace(node: GraphNode): Promise<GraphData> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to remove node from workspace: ${node.id} (${response.status})`,
      );
    }
  ...
  static async addAuthorsLatestTweets(authorId: string): Promise<GraphData> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to add top tweets for author: ${authorId} (${response.status})`,
      );
    }
   ...
   * TWEET
   */
  static async addTweetAuthorToWorkspace(
    tweetNode: GraphNode,
  ): Promise<GraphData> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to add tweet author to workspace: ${tweetNode.id} (${response.status})`,
      );
    }
  ...
  static async addTweetHashtagsToWorkspace(
    tweetNode: GraphNode,
  ): Promise<GraphData> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to add tweet hashtags to workspace: ${tweetNode.id} (${response.status})`,
      );
    }
  ...
  static async addMentionedAuthorsToWorkspace(
    tweetNode: GraphNode,
  ): Promise<GraphData> {
    const response = await fetch(
      `${API_BASE_URL}/graph/menu/tweet/mentioned-authors`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to add mentioned authors to workspace: ${tweetNode.id} (${response.status})`,
      );
    }
  ...
  static async highlightUsersForHashtag(
    hashtagNode: GraphNode,
  ): Promise<GraphData> {
    const response = await fetch(
      `${API_BASE_URL}/graph/menu/hashtag/highlight-authors`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to highlight authors for hashtag: ${hashtagNode.id} (${response.status})`,
      );
    }
  ...
  static async addTopTweetsByHashtag(
    hashtagNode: GraphNode,
  ): Promise<GraphData> {
    const response = await fetch(
      `${API_BASE_URL}/graph/menu/hashtag/top-tweets`,
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to add top tweets for hashtag: ${hashtagNode.id} (${response.status})`,
      );
    }
