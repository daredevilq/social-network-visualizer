  ...
  const fetchData = async () => {
    ...
    try {
      const userDataRes = await fetch(
        `${API_BASE_URL}/author/${selectedUserData.name}`,
      );
      if (!userDataRes.ok) throw new Error("Failed to load user data");
      const data = await userDataRes.json();
      setUserData(data);

      const lastPostsRes = await fetch(
        `${API_BASE_URL}/author/last-posts/${selectedUserData.name}`,
      );
...
"
          ...
          {loading ? (
            <div className="grid grid-cols-2 gap-3">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="h-16 bg-[#3D3D4E] rounded-lg animate-pulse"
                />
              ))}
            </div>
          ) : userData ? (
        ...
        <div className="bg-[#32323F] rounded-xl p-4 shadow-md flex-1 max-h-fit backdrop-blur-sm border border-[#3D3D4E]/50 flex flex-col min-h-0">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() =>
                runWithUnsavedCheck(async () => showUserCommunity())
              }
              className="px-3 py-1 text-xs bg-[#7140F4] hover:bg-[#5c32c3] text-white rounded-md transition-all duration-300 shadow-lg hover:shadow-xl cursor-pointer
"
            >
            ...
            {loading ? (
              <div className="space-y-4">
                {[...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className="h-24 bg-[#3D3D4E] rounded-lg animate-pulse"
                  />
                ))}
              </div>
              ...
              <div className="space-y-3">
                {lastPosts.map(({ url, contentPreview }, index) => (
                  ...
                  >
                    ...
                    >
                      {contentPreview
                        ? truncatePreview(contentPreview)
              ...
              <div className="flex flex-col items-center justify-center h-48 text-center bg-[#3D3D4E]/30 rounded-lg border border-dashed border-[#3D3D4E]">
                ...
                <p className="text-xs text-gray-500">
                  This user has no prominent posts
                </p>
              </div>
            )}
            <div className="mt-3">
              <button
                onClick={() =>
                  runWithUnsavedCheck(async () =>
                    router.push(`/tweet-analysis/${selectedUserData?.name}`),
                  )
                }
