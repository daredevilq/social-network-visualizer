...
interface BaseActivityTimelineProps {
  title: string;
  activityData: any | null;
  chartData: ChartData<'line'>;
}

export default function BaseActivityTimeline({ title, activityData, chartData }: BaseActivityTimelineProps) {
  const lineChartData = {
    ...chartData,
  ...
  return (
    <div className="bg-[#2A2D3D] rounded-xl p-6 shadow-lg border border-gray-700/50">
      <h2 className="text-xl font-bold text-white mb-6 flex items-center">{title}</h2>
