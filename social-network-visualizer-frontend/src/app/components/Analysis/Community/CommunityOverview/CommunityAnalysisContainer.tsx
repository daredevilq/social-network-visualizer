...
export default function CommunityAnalysisContainer() {
  ...
  const { list, loading: loadList, hasMore, fetchPage, page } = useCommunityList();

  const { ref, inView } = useInView({ rootMargin: '200px' });
  React.useEffect(() => {
    if (inView && hasMore && !loadList) fetchPage(page);
    ...
    <section className="flex flex-col w-full min-h-screen bg-[#262631] text-[#FAFAFA] p-6">
      <div className="flex flex-col h-full pt-8 pb-8 max-w-7xl mx-auto w-full">
        <header className="relative flex items-center w-full mb-6">
          ...
          <h1 className="absolute left-1/2 -translate-x-1/2 text-2xl md:text-3xl font-bold">Community analysis</h1>
        </header>
