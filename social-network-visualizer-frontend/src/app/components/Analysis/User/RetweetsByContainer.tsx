...
import { Repeat, User2 } from 'lucide-react';

interface RetweetsByContainerProps {
  retweetedUsers: string[];
}

export function RetweetsByContainer({ retweetedUsers }: RetweetsByContainerProps) {
  const router = useRouter();
  const hasData = retweetedUsers && retweetedUsers.length > 0;

  return (
    <div className="bg-[#2A2D3D] rounded-xl p-6 shadow-lg border border-white/5 flex flex-col h-full w-full">
      <div className="flex items-center mb-6 border-b border-white/10 pb-4">
        <Repeat className="w-5 h-5 mr-2 text-[#7140F4]" />
        <h2 className="text-xl font-bold text-white">
          Users Retweeted <span className="text-gray-500 ml-1 text-base font-normal">({retweetedUsers?.length || 0})</span>
        </h2>
      </div>

      {hasData ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {retweetedUsers.map((user, index) => (
            <div
              key={index}
        ...
        <div className="flex flex-col items-center justify-center h-40 text-gray-400">
          <Repeat className="w-8 h-8 mb-3 opacity-20" />
