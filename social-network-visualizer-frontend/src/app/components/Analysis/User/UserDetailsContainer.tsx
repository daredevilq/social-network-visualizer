    ...
    if (userData && !loading && !animationStarted.current) {
      ...
      const timer = setInterval(() => {
        ...
        setAnimatedStats({
          ...
          averageLikesCount:
            Math.round(targets.averageLikesCount * progress * 100) / 100,
          averageRepliesCount:
            Math.round(targets.averageRepliesCount * progress * 100) / 100,
          averageRetweetsCount:
            Math.round(targets.averageRetweetsCount * progress * 100) / 100,
        });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const activityRes = await fetch(
        `${API_BASE_URL}/author/activity/${username}`,
      );
      ...
      const retweetsByRes = await fetch(
        `${API_BASE_URL}/author/retweets-by/${username}`,
      );
      ...
      const retweetsOfRes = await fetch(
        `${API_BASE_URL}/author/retweets-of/${username}`,
      );
      ...
      const viralTweetsRes = await fetch(
        `${API_BASE_URL}/author/viral-tweets/${username}`,
      );
      ...
      const mostCommonWordsRes = await fetch(
        `${API_BASE_URL}/author/most-common-words/${username}`,
      );
      ...
      const userHeatMapRes = await fetch(
        `${API_BASE_URL}/author/heatmap/${username}`,
      );
      if (!userHeatMapRes.ok) throw new Error("Failed to load user data");
      const userHeatMapData: ActivityHeatmap[] =
        (await userHeatMapRes.json()) as ActivityHeatmap[];
      setUserHeatMap(userHeatMapData);
    } catch (err: any) {
      const message =
        err?.message || "Unexpected error occurred while fetching user data";
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <UsersMentionedContainer
              ...
              message={"Users Mentioned by this User"}
            />
