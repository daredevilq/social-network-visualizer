...
class WorkspaceServiceTest {
  ...
  void testGetAllWorkspaces_ProjectNotFound() {
    ...
    ProjectException exception = assertThrows(
        ...
        () -> workspaceService.getAllWorkspaces("nonExistent")
    );
  ...
  void testGetAllWorkspaces_NoWorkspaces() {
    // Arrange
    Project projectWithNoWorkspaces = Project.builder()
        .name("testProject")
        .workspaces(new ArrayList<>())
        .build();
    when(projectRepository.findByName("testProject")).thenReturn(Optional.of(projectWithNoWorkspaces));
  ...
  void testGetAllWorkspaces_NullWorkspaces() {
    // Arrange
    Project projectWithNullWorkspaces = Project.builder()
        .name("testProject")
        .workspaces(null)
        .build();
    when(projectRepository.findByName("testProject")).thenReturn(Optional.of(projectWithNullWorkspaces));
  ...
  void testSaveWorkspace_NewWorkspace() {
    ...
    assertTrue(testProject.getWorkspaces().stream()
        .anyMatch(w -> "newWorkspace".equals(w.getName())));
  ...
  void testSaveWorkspace_BlankWorkspaceName() {
    // Arrange
    Workspace workspaceWithBlankName = Workspace.builder()
        .name("   ")
        .nodes(new ArrayList<>())
        .build();
  ...
  void testDeleteWorkspace_NullWorkspaceName() {
    ...
    ProjectException exception = assertThrows(
        ProjectException.class,
        () -> workspaceService.deleteWorkspace("testProject", null)
    );
    assertEquals("Workspace name cannot be null or empty", exception.getMessage());
  }
  ...
  void testGetWorkspaceByName_Success() {
    // Arrange
    when(projectRepository.findWorkspaceByProjectNameAndWorkspaceName("testProject", "testWorkspace"))
  ...
  void testGetWorkspaceByName_NullName() {
    ...
    ProjectException exception = assertThrows(
        ProjectException.class,
        () -> workspaceService.getWorkspaceByName("testProject", null)
    );
    assertEquals("Workspace name cannot be null or empty", exception.getMessage());
  }
  ...
  void testGetWorkspaceByNameWithFullNodes_Success() {
    ...
    when(projectRepository.findWorkspaceByProjectNameAndWorkspaceName("testProject", "testWorkspace"))
        .thenReturn(Optional.of(testProject));
    when(authorRepository.findFullAuthorNodesByIds(anySet()))
        .thenReturn(List.of(authorNode));
    when(tweetRepository.findFullTweetNodesByIds(anySet()))
        .thenReturn(List.of(tweetNode));

    // Act
    Workspace result = workspaceService.getWorkspaceByNameWithFullNodes("testProject", "testWorkspace");
  ...
  void testGetWorkspaceByNameWithFullNodes_EmptyNodes() {
    // Arrange
    Workspace workspaceWithNoNodes = Workspace.builder()
        .name("emptyWorkspace")
        .nodes(new ArrayList<>())
        .edges(new ArrayList<>())
        .build();
    Project projectWithEmptyWorkspace = Project.builder()
        .name("testProject")
        .workspaces(new ArrayList<>(List.of(workspaceWithNoNodes)))
        .build();
    when(projectRepository.findWorkspaceByProjectNameAndWorkspaceName("testProject", "emptyWorkspace"))
        .thenReturn(Optional.of(projectWithEmptyWorkspace));

    // Act
    Workspace result = workspaceService.getWorkspaceByNameWithFullNodes("testProject", "emptyWorkspace");
  ...
  void testValidateAndImportWorkspace_Success() throws Exception {
    // Arrange
    String workspaceJson = "{\"name\":\"importedWorkspace\",\"nodes\":[{\"id\":\"author1\",\"nodeType\":\"AUTHOR\"}],\"edges\":[]}";
  ...
  void testValidateAndImportWorkspace_NoNodesAndEdges() throws Exception {
    ...
    Workspace workspaceWithNoData = Workspace.builder()
        .name("test")
        .nodes(null)
        .edges(null)
        .build();
    ...
    assertTrue(exception.getMessage().contains("No valid nodes and edges found in the workspace file"));
  }
  ...
  void testValidateAndImportWorkspace_DuplicateWorkspaceName() throws Exception {
    ...
    Workspace duplicateWorkspace = Workspace.builder()
        .name("testWorkspace")
        .nodes(new ArrayList<>())
        .edges(new ArrayList<>())
        .build();
  ...
  void testValidateAndImportWorkspace_NoValidNodesOrEdges() throws Exception {
    ...
    MockMultipartFile file = new MockMultipartFile(
        ...
        "workspace.json",
        "application/json",
        "{\"name\":\"newWorkspace\",\"nodes\":[{\"id\":\"invalid\",\"nodeType\":\"AUTHOR\"}],\"edges\":[]}".getBytes()
    );
  ...
  void testValidateAndImportWorkspace_WithValidEdges() throws Exception {
    // Arrange
    String workspaceJson = "{\"name\":\"newWorkspace\",\"nodes\":[{\"id\":\"author1\",\"nodeType\":\"AUTHOR\"},{\"id\":\"tweet1\",\"nodeType\":\"TWEET\"}],\"edges\":[{\"source\":\"author1\",\"target\":\"tweet1\",\"relationType\":\"POSTED\"}]}";
    MockMultipartFile file = new MockMultipartFile(
        "workspace",
        "workspace.json",
        "application/json",
    ...
    when(graphRepository.findExistingRelations(anyList()))
        .thenReturn(List.of(testLink));
    when(projectRepository.save(any(Project.class))).thenAnswer(invocation -> invocation.getArgument(0));
  ...
  void testValidateAndImportWorkspace_FilterInvalidEdges() throws Exception {
    ...
    String workspaceJson = "{\"name\":\"newWorkspace\",\"nodes\":[{\"id\":\"author1\",\"nodeType\":\"AUTHOR\"}],\"edges\":[{\"source\":\"invalid1\",\"target\":\"invalid2\",\"relationType\":\"POSTED\"}]}";
    MockMultipartFile file = new MockMultipartFile(
        "workspace",
        "workspace.json",
        "application/json",
